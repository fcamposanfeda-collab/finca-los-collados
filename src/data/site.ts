export const site = {
  name: 'Finca Los Collados de San Miguel',
  shortName: 'Los Collados',
  logoLines: ['Finca', 'Los Collados', 'de San Miguel'] as const,
  url: 'https://fcamposanfeda-collab.github.io/finca-los-collados',
  lang: 'es',
  copyright: '© 2026 Finca Los Collados de San Miguel. Todos los derechos reservados.',
  tagline: 'Turismo rural, empresas y eventos en Campos de Montiel',
  ogImage: '/images/hero-finca.jpg',
  bookingUrl: '/reservas-online',
} as const;
